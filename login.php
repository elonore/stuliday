<?php
    ...
    //(password_verify($user_pass, $db_pass) supposant que $db_pass corresdpond au mot de passe hashé et que $user_pass correspond au mot de passe posté)
    ...
    if (isset($_POST['submit-signup'])){
        ...
        if($sql = $db->query("SELECT * FROM users WHERE email = '$user_email'")){
            ...
            if($compteur != 0){
                ...
                if($sth->execute()){
                    $message = "<div class ='alert alert-success'> Votre compte a bien été créé </div>";
                    }
                }else{
  ...
  /*requête de sélection de l'email user en fonction de  l'input user*/
    if($row = $sql->fetch()){
        ...
        if(password_verify($user_pass, $db_pass)){
            ...
            $message = "<div class ='alert alert-success'> La connexion  a bien été effectuée </div>";
                    }
        }else{
